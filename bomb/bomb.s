...
0000000000400da0 <main>:
  ...
  400e37:	48 89 c7             	mov    %rax,%rdi            ///////phase_1//////这里，read_line的返回值rax被传入rdi
  400e3a:	e8 a1 00 00 00       	callq  400ee0 <phase_1>
  400e3f:	e8 80 07 00 00       	callq  4015c4 <phase_defused>
  400e44:	bf a8 23 40 00       	mov    $0x4023a8,%edi
  400e49:	e8 c2 fc ff ff       	callq  400b10 <puts@plt>
  400e4e:	e8 4b 06 00 00       	callq  40149e <read_line>
  400e53:	48 89 c7             	mov    %rax,%rdi            ///////phase_2//////这里，read_line的返回值rax被传入rdi
...
0000000000400ee0 <phase_1>:
  ...
  400ee4:	be 00 24 40 00       	mov    $0x402400,%esi  /////////////实际上是rsi的一部分, 功能上就是第二个参数
  400ee9:	e8 4a 04 00 00       	callq  401338 <strings_not_equal> // 大胆猜测strings_not_equal(%rdi, %esi) return %rax;
  400eee:	85 c0                	test   %eax,%eax               ////////////进行检定，
...
0000000000400efc <phase_2>:
  ...
  400f0e:	74 20                	je     400f30 <phase_2+0x34>             ////////////////////jump if equal
  400f10:	e8 25 05 00 00       	callq  40143a <explode_bomb>
  400f15:	eb 19                	jmp    400f30 <phase_2+0x34>
  400f17:	8b 43 fc             	mov    -0x4(%rbx),%eax  // 循环开头，因为rbx先+4后-4，因此eax存储的仍然是rsp的原始值
  400f1a:	01 c0                	add    %eax,%eax // 核心语句，这道题的答案就是“输入是否等于上一个输入的2倍”
  ...
  400f30:	48 8d 5c 24 04       	lea    0x4(%rsp),%rbx // rbx存储的仍然是地址
  400f35:	48 8d 6c 24 18       	lea    0x18(%rsp),%rbp // rbp仍然是地址
  ...
  函数phase_2()
  {
    ...
    if((%rsp) == $0x1)
    {
      %rbx = (0x4 + %rsp); // lea    0x4(%rsp),%rbx // rbx存储的仍然是地址
      %rbp = (0x18 + %rsp); // lea    0x18(%rsp),%rbp // rbp仍然是地址
      然后前往循环开头; // jmp    400f17 <phase_2+0x1b>
    }else{
      引爆炸弹;
    }
    ...
    while() // 循环开头 400f17
    {
      eax = (rbx - 0x4); // mov    -0x4(%rbx),%eax   这意味着先去寻址(rbx目前为rsp + 0x4，因此eax最终会去内存地址为rsp的地方读取)，意味着从内存中取出一个数，并将其存储到 %eax 中
      eax = eax + eax; // 令eax加倍
      if((%rbx) = eax) // 此时%rbx为rsp + 0x4，并且此时%eax为2倍的(rsp)，因此，你可以在此处得到本题的答案“想要炸弹不爆炸，你得让(rsp + 0x4)与2倍的(rsp)相等”
      {                // rsp存储的是用户输入的6个数字的第一个数字的起始地址，rsp + 0x4是用户输入的第二个数字的起始地址
        脱离if语句;
      }else{
        引爆炸弹;
      }
      ...
      if(rbx != rbp)  // cmp    %rbp,%rbx
      {
        continue; // 回到循环的开头; jne    400f17 <phase_2+0x1b>
      }else{ // 一旦rbx == rbp 就可以不引爆炸弹的结束
        无条件跳转，也就是说，直接执行下方语句; //jmp    400f3c <phase_2+0x40>
        400f3c:	48 83 c4 28          	add    $0x28,%rsp
        400f40:	5b                   	pop    %rbx
        400f41:	5d                   	pop    %rbp
        400f42:	c3                   	retq
...
0000000000400f43 <phase_3>:
  ...
  400f5b:	e8 90 fc ff ff       	callq  400bf0 <__isoc99_sscanf@plt> // 他会返回输入的数量
  400f60:	83 f8 01             	cmp    $0x1,%eax // 因此你必须输入至少2个参数
  400f63:	7f 05                	jg     400f6a <phase_3+0x27>
  400f65:	e8 d0 04 00 00       	callq  40143a <explode_bomb> // 不然就会爆炸
  400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) // 如果0x7大于用户输入的第1个参数
  400f6f:	77 3c                	ja     400fad <phase_3+0x6a> // ，那么就会跳转到炸弹爆炸，所以，我们希望用户的输入 <= 0x7
  400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax        // 0x8(%rsp)所指示的内存地址处存储了用户的第1个输入
  400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)    // 0x402470 处存储的是0x00400f7c,也就是说这个语句实际上是 400f7c + eax*8 ，别忘了eax是rax的一部分
  ...
  400fb2:	b8 00 00 00 00       	mov    $0x0,%eax   // 解题的正确答案在于想办法跳到 0x 400fb2 这里来，只要输入7就能跳转到这里来
...
7 327
这道题的逻辑是：
  先用gdb调试器查看内存0x4025cf处的数据，一看0x4025cf:       "%d %d" 说明用户输入应为2个decimal值
  然后下面这2个指令提示你不要用户的第一个输入不应该大于7
// 400f6a:	83 7c 24 08 07       	cmpl   $0x7,0x8(%rsp) // 如果0x7大于用户输入的第1个参数
// 400f6f:	77 3c                	ja     400fad <phase_3+0x6a> // ，那么就会跳转到炸弹爆炸，所以，我们希望用户的输入 <= 0x7
强烈暗示了这是一个Switch结构编译来的汇编代码

  之后查看下列代码，
// 400f71:	8b 44 24 08          	mov    0x8(%rsp),%eax        // 0x8(%rsp)所指示的内存地址处存储了用户的第1个输入
  我们用gdb查看0x8(%rsp)存的什么，一看是用户输入的第1个参数

  然后查看下列代码：
// 400f75:	ff 24 c5 70 24 40 00 	jmpq   *0x402470(,%rax,8)  // 0x402470 处存储的是0x00400f7c,也就是说这个语句实际上是 400f7c + eax*8 ，别忘了eax是rax的一部分
  *0x402470的意思是“请把我当成一个内存地址看待”，也就是说我们得去内存的0x402470处看看那里有什么，一看是400f7c，刚好就是下一行代码
  于是*0x402470(,%rax,8)的意思就是400f7c + eax*8
  非常符合Switch语句的跳转逻辑（跳转表）

  这个时候，只要你的第1个参数 <= 0x7，那么你就一定会经过
// 400fbe:	3b 44 24 0c          	cmp    0xc(%rsp),%eax
  现在已经2024-12-9 01:48:14了，所以我简单点说：
  0xc(%rsp)是用户输入的第2个参数，Switch结构中每个case中的语句都会改变%eax，
  总之你的目的是让“用户输入的第2个参数” 等于 “寄存器%eax中存储的值”
  你可以用gdb调试器在0x400fbe处打一个断点，然后info r eax，来获得此时eax存储的值，这就是用户输入第2个参数的答案了。

2024-12-9 01:51:08 至此本题结束。
...
000000000040100c <phase_4>:
  ...
  401010:	48 8d 4c 24 0c       	lea    0xc(%rsp),%rcx  // 第二个参数送到rcx
  401015:	48 8d 54 24 08       	lea    0x8(%rsp),%rdx  // 第一个参数送到rdx
  40101a:	be cf 25 40 00       	mov    $0x4025cf,%esi // 这不用想肯定是用户输入的规定，用x/s 查看一下 0x4025cf:       "%d %d"
  40101f:	b8 00 00 00 00       	mov    $0x0,%eax
  401024:	e8 c7 fb ff ff       	callq  400bf0 <__isoc99_sscanf@plt>
  401029:	83 f8 02             	cmp    $0x2,%eax
  40102c:	75 07                	jne    401035 <phase_4+0x29>
  40102e:	83 7c 24 08 0e       	cmpl   $0xe,0x8(%rsp) // if 0x8(%rsp) <= 0xe
  401033:	76 05                	jbe    40103a <phase_4+0x2e> // 则跳转
  401035:	e8 00 04 00 00       	callq  40143a <explode_bomb>  // 因此第1个参数必须<= 0xe
  40103a:	ba 0e 00 00 00       	mov    $0xe,%edx // 也就是说这里改变了rdx
  40103f:	be 00 00 00 00       	mov    $0x0,%esi // 也就是说这里改变了rsi
  401044:	8b 7c 24 08          	mov    0x8(%rsp),%edi // 目前为止出现的寄存器有rcx rdx esi eax,你尤其要关注0x8(%rsp) 和 0xc(%rsp)
  401048:	e8 81 ff ff ff       	callq  400fce <func4>

  // 你得关注 edx esi

  40104d:	85 c0                	test   %eax,%eax                // eax若不为0，就会跳转然后引爆炸弹
  40104f:	75 07                	jne    401058 <phase_4+0x4c>    // 因此我们希望eax为0x0
  401051:	83 7c 24 0c 00       	cmpl   $0x0,0xc(%rsp) 
  401056:	74 05                	je     40105d <phase_4+0x51>    // if(第2个参数 == 0x0)
  401058:	e8 dd 03 00 00       	callq  40143a <explode_bomb>
  40105d:	48 83 c4 18          	add    $0x18,%rsp               // 则不会引爆炸弹
  401061:	c3                   	retq   
到目前为止，我们知道的信息有：
第1个参数 <= 0xe
第2个参数为0x0
我们希望执行到40104d的时候，eax 为0
2024-12-9 18:04:05 也就是说第1个参数只能是7
// 目前为止出现的寄存器有 rcx rdx esi eax,
// 你尤其要关注0x8(%rsp) 和 0xc(%rsp)即 rdx 和 rcx
edx = 0xe
esi = 0x0
edi = 第1个参数
0000000000400fce <func4>:
  400fce:	48 83 ec 08          	sub    $0x8,%rsp 
  400fd2:	89 d0                	mov    %edx,%eax // edx 里存储的是0xe
  400fd4:	29 f0                	sub    %esi,%eax // eax = eax - esi 来，和我一起念：目标操作数 sub(减去) 源操作数
  400fd6:	89 c1                	mov    %eax,%ecx // eax = 0xe 因此 ecx现在为 0xe
  400fd8:	c1 e9 1f             	shr    $0x1f,%ecx // 目标操作数ecx 右移 0x1f
  400fdb:	01 c8                	add    %ecx,%eax // eax = eax + ecx
  400fdd:	d1 f8                	sar    %eax // sar是算数右移(shift arithmetic right),由cl寄存器控制(%ecx的低8位)
  400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx // ecx = rax + 1*rsi
  400fe2:	39 f9                	cmp    %edi,%ecx // 源操作数edi less equal than 目标操作数ecx
  400fe4:	7e 0c                	jle    400ff2 <func4+0x24>
  400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx // 这里操作了第二个参数
  400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>
  400fee:	01 c0                	add    %eax,%eax
  400ff0:	eb 15                	jmp    401007 <func4+0x39>
  400ff2:	b8 00 00 00 00       	mov    $0x0,%eax
  400ff7:	39 f9                	cmp    %edi,%ecx
  400ff9:	7d 0c                	jge    401007 <func4+0x39>
  400ffb:	8d 71 01             	lea    0x1(%rcx),%esi // 这里操作了第二个参数
  400ffe:	e8 cb ff ff ff       	callq  400fce <func4>
  401003:	8d 44 00 01          	lea    0x1(%rax,%rax,1),%eax
  401007:	48 83 c4 08          	add    $0x8,%rsp
  40100b:	c3                   	retq   
...
0000000000400fce <func4>()
{
  eax = edx; // 400fd2:	89 d0                	mov    %edx,%eax
  eax = eax - esi;
  ecx = eax; // 400fd6:	89 c1                	mov    %eax,%ecx
  ecx 右移 0x1f ;  // ecx为32位，逻辑右移31位，这相当于只留下最高位 400fd8                  shr    $0x1f,%ecx
  eax = eax + ecx ; // 400fdb:	01 c8                	add    %ecx,%eax 
  eax 算数右移 ; // 这里什么都没做，因为 %cl里存的是0x0  sar是算数右移(shift arithmetic right),由cl寄存器控制(%ecx的低8位)
  ecx = rax + 1*rsi ; // 400fdf:	8d 0c 30             	lea    (%rax,%rsi,1),%ecx 
  
  ecx 现在是 7
  if (edi <= ecx) // 400fe2 400fe4
  {
    eax = 0x0 ;                        // 400ff2: mov    $0x0,%eax
    if(edi >= ecx) // 400ff7 400ff9
    {
      jge    401007 <func4+0x39>
    }
  }else{
    edx = -0x1 + rcx;                      // 400fe6:	8d 51 ff             	lea    -0x1(%rcx),%edx // 这里操作了第二个参数
    递归调用0000000000400fce <func4>();     // 400fe9:	e8 e0 ff ff ff       	callq  400fce <func4>
    eax = eax + eax;                        // 400fee:	01 c0                	add    %eax,%eax
    jmp    401007 <func4+0x39> ;            // 00ff0:	eb 15                	jmp    401007 <func4+0x39>
  }
  401007:	48 83 c4 08          	add    $0x8,%rsp
  40100b:	c3                   	retq   
}
...
0000000000401062 <phase_5>:
  ...
  40107a:	e8 9c 02 00 00       	callq  40131b <string_length> // 该函数暗示你用户输入存进了(%rdi)
  40107f:	83 f8 06             	cmp    $0x6,%eax // 这暗示了用户输入必须为6个字符长
  ...
  这里是一个while循环
  { // 第一次循环时, eax = 0x0
  40108b:	0f b6 0c 03          	movzbl (%rbx,%rax,1),%ecx    // move with zero extension(byte to long) 从计算出的地址中读取一个字节(8bits)数据。ecx 和 rcx 有关
  // 一看gdb, rax 是 0, 也就是说他直接把rbx(用户输入)的前8位赋给了ecx
  ...
  while(rax <= 0x6)
  {
    ecx = rbx(用户输入)的前8位                 // 40108b ecx 是 rcx 的低32位, ecx 是用户输入
    (%rsp) = %cl                              // 40108f  cl 是 ecx 的低8位, cl 是 rcx 的低8位 , (%rsp) 是用户输入
    rdx = (%rsp)                              // 401092 rdx 是用户输入
    0xf ^ %edx                                // 401096 也就是只保留edx的前4位, edx 是 rdx 的低32位, edx 是用户输入
    edx = 0x4024b0(%rdx)的第1个字节，使用0扩展  // 401099 edx 是 rdx 的低32位, rdx 是用户输入
    0x10(%rsp,%rax,1) = dl                    // 4010a4 这！就是那个答案数组！    dl 是 edx / rdx(用户输入) 的低8位
    rax = rax + 0x1;
  }
  逻辑链:
  我们想改变 rdi ,最终目标是让 rdi != esi 从而避免炸弹爆炸
  我们可以通过 rbx (用户输入) 来操控 ecx ,因为cl是ecx的低8位,因此我们可以通过用户输入 rbx (用户输入) 操控 cl
  根据 40108f , 我们可以用 cl 来操控 (%rsp)
  4010b8,最终我们可以操控 rdi, 也就是说, 我们的 rbx(用户输入) 可以操控 rdi
...
我们希望是: flyers
2024-12-10 20:02:37 
关键在于你得用低4位为15 (1111) 的字母来表达字符数组的第15个字母
关键在于你得用低4位为14 (1110) 的字母来表达字符数组的第14个字母


  4010ae:	c6 44 24 16 00       	movb   $0x0,0x16(%rsp)
  4010b3:	be 5e 24 40 00       	mov    $0x40245e,%esi   // esi 无法改变，他固定是 "flyers"
  4010b8:	48 8d 7c 24 10       	lea    0x10(%rsp),%rdi  // 因此我们得想办法改变 rdi, 让 rdi == esi
  4010bd:	e8 76 02 00 00       	callq  401338 <strings_not_equal>   // strings_not_equal(%rdi, %esi) 若不相等 return eax = 1, 若相等 return eax = 0;
  4010c2:	85 c0                	test   %eax,%eax                    // 因此, 我们希望zf = 1
  4010c4:	74 13                	je     4010d9 <phase_5+0x77>        // 也就是说我们希望 eax 或 rax = 0,也就是我们希望 rdi == esi
...
循环
{
  ...
  401117:	41 8b 45 00          	mov    0x0(%r13),%eax // 此时eax存储的是用户的第1个输入, 他把 rsp (也就是用户输入的起始地址)赋给了 r13 rsi r14 rbp 
  40111b:	83 e8 01             	sub    $0x1,%eax
  40111e:	83 f8 05             	cmp    $0x5,%eax      // eax <= 0x5
  401121:	76 05                	jbe    401128 <phase_6+0x34>
  401123:	e8 12 03 00 00       	callq  40143a <explode_bomb><explode_bomb><explode_bomb><explode_bomb>
  401128:	41 83 c4 01          	add    $0x1,%r12d
  40112c:	41 83 fc 06          	cmp    $0x6,%r12d
  401130:	74 21                	je     401153 <phase_6+0x5f>
  401132:	44 89 e3             	mov    %r12d,%ebx    // 40110e 说明 r12d 是0, 因此 ebx 也是 0

  401135:	48 63 c3             	movslq %ebx,%rax
  401138:	8b 04 84             	mov    (%rsp,%rax,4),%eax
  40113b:	39 45 00             	cmp    %eax,0x0(%rbp)       // 不可以和用户的第1个输入相同
  40113e:	75 05                	jne    401145 <phase_6+0x51>
  401140:	e8 f5 02 00 00       	callq  40143a <explode_bomb><explode_bomb><explode_bomb><explode_bomb>
...
for(eax != ecx, eax++)
{
  401176:	48 8b 52 08          	mov    0x8(%rdx),%rdx // $rdx+0x4 是用户的第1个输入
...
ecx = 7
这里是关键，只要把这里的逻辑搞清楚就能解题
while(0x1 <= ecx)
{
  401183:	ba d0 32 60 00       	mov    $0x6032d0,%edx
  401188:	48 89 54 74 20       	mov    %rdx,0x20(%rsp,%rsi,2) // rsi 目前为 0x18(%rsp) = 0, 我有很大的概率可以相信 rsi 永远都是0
  ...
  4011ab:	48 8b 5c 24 20       	mov    0x20(%rsp),%rbx // rbx+0x4 存储一个地址a，地址a指向另一个地址b，地址b指向用户的第1个输入
...
// 他把 rsp (也就是用户输入的起始地址) 赋给了 r13 rsi r14 rbp , 他为什么要这么做? 
ebp = 5
while(ebp != 0)
{
  4011df:	48 8b 43 08          	mov    0x8(%rbx),%rax // rax+0x4 指向用户的第5个输入
  4011e3:	8b 00                	mov    (%rax),%eax    // eax 是 rax 的低32位, 这个mov并没有改变任何东西
  4011e5:	39 03                	cmp    %eax,(%rbx)   // rbx+0x4 指向用户的第6个输入
  4011e7:	7d 05                	jge    4011ee <phase_6+0xfa> // jump if[ (%rbx) >= %eax ]
  4011e9:	e8 4c 02 00 00       	callq  40143a <explode_bomb><explode_bomb><explode_bomb><explode_bomb>
...
0000000000401338 <strings_not_equal>: // 可能是strings_not_equal(%rdi, %esi) return %rax;
  401338:	41 54                	push   %r12
  40133a:	55                   	push   %rbp
  40133b:	53                   	push   %rbx
  40133c:	48 89 fb             	mov    %rdi,%rbx
  40133f:	48 89 f5             	mov    %rsi,%rbp // 虽然我们录入第二个参数的时候使用的是esi,但是esi是rsi的一部分,所以你也可以通过rsi来使用第二个参数
...
000000000040145c <read_six_numbers>:
  ...
  401460:	48 89 f2             	mov    %rsi,%rdx  ///////在400f02这一行，rsi的值被定义为rsp - 0x28
  ...
  401499:	48 83 c4 18          	add    $0x18,%rsp   ///回收栈空间        //////////// 大概逻辑是，先比较eax和0x5的值，
  40149d:	c3                   	retq                                    //////////// 我们希望eax == 0x5，这样炸弹就不会爆炸
